...
- `len(arr.markers)` becomes `int(np.count_nonzero(arr.markers))`.
- `for ind in arr.markers: ...` becomes `for ind in np.argwhere(arr.markers): ...`.
- `arr.markers = [[0,1], [0,2]]` is still accepted and auto-converted.
- `np.array_equal(arr.markers, m)` is unchanged when `m` is also boolean.
