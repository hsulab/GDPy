...
runtimes. When broadcast runtimes are used as steps of a `runtime_chain`,
variants are paired by index and scalar steps are repeated. All broadcast
steps in one chain must therefore expand to the same number of variants.
