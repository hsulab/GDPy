        ...
        if not hasattr(self.comparator, "prepare_data"):
            ...
            for i, fp in enumerate(fingerprints):
                for j in selected_indices[::-1]:
                    ...
                    fp_avg = np.average([fingerprints[x] for x in unique_groups[j]], axis=0)
            ...
            for k, v in unique_groups.items():
                unique_fingerprints[k] = np.average([fingerprints[x] for x in v], axis=0)

            selected_group_indices, merged_groups = [], {}
            for i, fp in unique_fingerprints.items():
                for j in selected_group_indices:
                    # TODO: The ditribution maybe too wide?
                    fp_avg = np.average([unique_fingerprints[x] for x in merged_groups[j]], axis=0)
                    if self.comparator(fp, fp_avg):
            ...
            for k, v in merged_groups.items():
                curr_indices = sorted(list(itertools.chain(*[unique_groups[x] for x in v])))
                #print(curr_indices)
            ...
            for i, (k, v) in enumerate(unique_groups.items()):
                ...
                if curr_nframes > 1:
                    ...
                    for ia, a in enumerate(curr_frames[:]):
                        ...
                        images.append(image)
                    image_table = []
                    nrows = int(curr_nframes/4.)
                    for x in range(nrows):
                        image_table.append([images[im] for im in range(x*4, (x+1)*4)])
