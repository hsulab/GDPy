...
class CorrelationValidator(BaseValidator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(cutoff, (int, float)):
            self.cutoffs = [cutoff] * len(group_b)
        else:
            self.cutoffs = cutoff
    ...
    def _irun(self, data, prefix, labels):
        ...
        # self._print(f"mdtrajs: {mdtrajs}")

        traj = [atoms for atoms in mdtrajs[0] if atoms is not None]

        # Define adsorbates and adsorption sites
        first_atoms = traj[0]
        group_indices_a = evaluate_group_expression(first_atoms, self.group_a)
        site_groups = [evaluate_group_expression(first_atoms, g) for g in self.group_b]
        ...
        nlist = NeighborList([max(self.cutoffs) / 2] * num_atoms, skin=0.3, self_interaction=False, bothways=True)
            ...
            for i in group_indices_a:
                ...
                for j, o in zip(indices, offsets):
                    for g_idx, index_map in enumerate(site_index_maps):
                        ...
                        if distance <= self.cutoffs[g_idx]:
        ...
        self._print(f"{all_connectivities.shape=}")
