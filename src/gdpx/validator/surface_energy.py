...
EVA2TOJNM2 = (1.0 / (units.kJ / 1000.0)) / ((1.0 / units.m) ** 2)


class SurfaceEnergyValidator(BaseValidator):
    ...
    def run(self, dataset, worker=None, *args, **kwargs):
        ...
        if ref_grp is not None:
            ref_data = self._compute_surface_energy(
                self._process_data(ref_grp["bulk"]), self._process_data(ref_grp["surfaces"]), self.nsides
            )
        ...
        if pre_grp is not None:
            pre_data = self._compute_surface_energy(
                self._process_data(pre_grp["bulk"]), self._process_data(pre_grp["surfaces"]), self.nsides
            )
