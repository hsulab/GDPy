...
class RouletteBuilder(StructureModifier):
    ...
    def run(self, substrates=None, size: int = 1, *args, **kwargs) -> list[Atoms]:
        ...
        # Check if we have roulette history in active learning
        prev_chosen = []
        if self.use_memory:  # TODO: Check if this builder is in active?
            ...
            ...
        num_prev_chosen = len(prev_chosen)
        ...
        for i in test_chosen:
            ...
            if num_chosen >= size:
                break
            if i not in prev_chosen or num_prev_chosen >= num_structures:
                curr_chosen.append(i)
        assert num_chosen == size, f"The chosen indices: {curr_chosen}."
