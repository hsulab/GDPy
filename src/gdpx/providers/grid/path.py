...
from gdpx.execution.path import BaseStringReactor, StringReactorSetting
...
def interpolate_string(a, b, t, npts: int = 5, concat: bool = False):
    ...
    if concat:
        ...
        pts = np.vstack([np.linspace(a[0], b[0], npts), np.linspace(a[1], b[1], npts)]).T
...
class ZeroStringReactor(BaseStringReactor):
    ...
    def run(self, structures, read_ckpt: bool = True, *args, **kwargs):
        ...
        images = np.vstack([np.linspace(a[0], b[0], nimages), np.linspace(a[1], b[1], nimages)]).T

        images = optimise_string(
            images,
            self.calc._points,
            self.calc._z,
            self.calc._gradients[0],
            self.calc._gradients[1],
            steps=100,
            stepsize=0.01,
        )
        energies = griddata(self.calc._points, self.calc._z.flatten(), (images[:, 0], images[:, 1]), method="linear")[
            :, np.newaxis
        ]

        np.savetxt(
            self.directory / "pmf.dat",
            np.hstack([np.linspace(0, 1, nimages)[:, np.newaxis], images, energies]),
            fmt="%8.4f",
        )
        ...
        ax.plot(np.linspace(0, 1, nimages), energies, marker="o")
        ...
        ax.plot(images[:, 0], images[:, 1], linestyle="--")
