...
def rms_dict(x_ref: list[float], x_pred: list[float]):
    ...
    x_ref_ = np.array(x_ref)
    x_pred_ = np.array(x_pred)

    if np.shape(x_pred_) != np.shape(x_ref_):
        raise ValueError("WARNING: not matching shapes in rms")

    error_2 = (x_ref_ - x_pred_) ** 2
...
def parity_plot(
    emulation: tuple[str, Union[np.ndarray, np.ndarray]],
    reference: tuple[str, Union[np.ndarray, np.ndarray]],
    ax,
    prop: tuple[str, str] = ("energy", "[eV/atom]"),
...
PropInfo = namedtuple("PropInfo", ["xlabel", "ylabel", "title"])


def parity_plot_dict(
    emulation: Mapping[str, Union[np.ndarray, np.ndarray]],
    reference: Mapping[str, Union[np.ndarray, np.ndarray]],
    ax,
    prop_dict: dict = None,
    apex=[1.0],
    write_info=True,
