...
from .utils import node_symbol, show_nodes, unpack_node_name
...
def normalize(vector):
    return (
        vector / np.linalg.norm(vector) if np.linalg.norm(vector) != 0 else vector * 0
    )
...
class SingleAdsorptionSite(object):
    ...
    def _compute_position(
        ...
        for ci, n in enumerate(node_names):
            ...
            site_positions[ci] = atoms.positions[a_idx] + np.dot(
        ...
        if order == 1:
            ...
            tangent = site_positions[2] - (site_positions[0] + site_positions[1]) / 2.0
        else:
            ...
    ...
    def is_occupied(self, ads_elemnts):
        ...
        ads_graphs = [
            ads_graphs.subgraph(c) for c in nx.connected_components(ads_graphs)
        ]
        ...
        for idx, ads in enumerate(ads_graphs):
            ...
            full_ads = nx.ego_graph(
                self.graph, initial, radius=0, distance="ads_only"
            )  # all nodes in this adsorbate, equal ads?
            # print("full ads: ", full_ads.nodes())

            new_ads = nx.ego_graph(
                self.graph, initial, radius=1, distance="dist"
            )  # consider neighbour atoms
            new_ads = nx.Graph(
                nx.subgraph(self.graph, list(new_ads.nodes()))
            )  # return a new copy of graph

            # update attribute of this adsorbate
            for node in ads.nodes():
                new_ads.add_node(
                    node, central_ads=True
                )  # central means the adsorbate interacting with surface
    ...
    def adsorb(
        ...
        inserted_ads_indices = [
            i + natoms for i in range(natoms_ads) if adsorbate_[i].symbol != "H"
        ]

        # adsorbates that are already in the substrate
        local_ads_indices = [
            i for i in other_ads_indices if self.atoms[i].symbol != "H"
        ]
...
def compute_site_normal(
    atoms: Atoms, nl, site_indices: List[int], surf_indices: List[int]
):
    ...
    if site_order > 2:
        ...
        for i in nl.get_neighbors(site_indices[0])[0]:
            ...
            if (
                (i not in site_indices)
                and i in nl.get_neighbors(site_indices[1])[0]
                and i in surf_indices
                and i not in neighbor_atoms
            ):
        ...
        if len(neighbor_atoms) > 0:
            for i in neighbor_atoms:
                ...
                for a in site_atoms1:
                    if a != initial:
                        a_offset = nl.get_neighbors(initial)[1][
                            np.where(a == nl.get_neighbors(initial)[0])
                        ]
                        # print(a,np.dot(a_offset, atoms.get_cell())+atoms[a].position)
                        atom_array.append(
                            atoms[a].position + np.dot(a_offset, atoms.get_cell())[0]
                        )
                # print(atom_array)
                ...
                if (
                    0.85
                    < get_angle_cycle(atom_array[0], atom_array[1], atom_array[2])
                    < 1.2
                ):  # 48.73 to 68.79 degree
        ...
        if np.sum(np.fabs(normal)) < 1e-8:
            ...
            normal = nvec + np.array([0.0, 0.0, 2**0.5])
        ...
        if region is None:
            ...
            for i, a in enumerate(atoms):
                if a.symbol == species:
                    ...
                    if (
                        (ox + xl <= pos[0] <= ox + xh)
                        and (oy + yl <= pos[1] <= oy + yh)
                        and (oz + zl <= pos[2] <= oz + zh)
                    ):
                        valid_indices.append(i)
    else:
...
class SiteFinder(StruGraphCreator):
    ...
    def find(self, atoms_: Atoms, site_params: list, surface_mask: List[int] = None):
        ...
        normals, surface_mask = generate_normals(
            ...
            ads_indices,
            mask_elements=[],
            surf_norm_min=self.surface_normal,
            normalised=True,
            system=system,
        )
    ...
    def _generate_single_site(
        ...
        found_sites = self._generate_site(
            atoms, graph, nl, valid_indices, surface_mask, normals, coordination=cn
        )
    ...
    def _broadcast_site_params(self, params: dict) -> List[dict]:
        ...
        for key, cur_num in zip(keys, nums):
            assert (
                cur_num == 1 or cur_num == num
            ), f"Parameter {key} size is inconsistent."
    ...
    def _generate_site(
        ...
        for cycle in valid:
            ...
            local_graph = nx.ego_graph(
                site_graph,
                centre_node,
                radius=self.DIS_SURF2SURF,
                center=True,  # 2 is surf_surf_dis
                distance="dist",
            )  # go over framework
            ...
            # TODO: add CN4 site that is a square
                ...
                for g in new_site_graphs:
                    ...
                    assert (
                        len(node_names) == coordination
                    ), "The site order is incorrect."
            ...
            for node_names in site_node_names:
                site_ads = SingleAdsorptionSite(
                    ...
                    surf_indices=surface_mask,
                )
                sites.append(site_ads)

        return sites

    def process_site(self, atoms, graph, nl, site: List[int], radius=3):
