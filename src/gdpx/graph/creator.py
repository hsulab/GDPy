...
from .utils import bond_symbol, grid_iterator, node_symbol
...
class NeighGraphCreator:
    ...
    def __init__(self, covalent_ratio=1.0, skin=0.0, self_interaction=False, bothways=True):
        """"""
...
class StruGraphCreator:
    ...
    #: Distance [Ang].
    substrate_adsorbate_distance: float = 2.5
    ...
    def __init__(
        ...
        pbc_grid: tuple[int, int, int] = (2, 2, 0),
        graph_radius: int = 2,
        ...
        num_dimensions = len(pbc_grid)
        if num_dimensions != 3:
            raise Exception("The number of dimensions in pbc_grid must be 3.")
        self.pbc_grid = pbc_grid

        # The half neighbour list (bothways=False) is enough for creating structure graph,
        # and no self-interaction is needed.
    ...
    def add_atoms_node(self, graph, sym1, a1, o1, **kwargs):
        ...
        """
        graph.add_node(node_symbol(sym1, a1, o1), index=a1, central_ads=False, **kwargs)

        return

    def add_atoms_edge(self, graph, sym1, sym2, a1, a2, o1, o2, dis, adsorbate_atoms, **kwargs):
        """graph edge format
        dist
            2 bond in substrate
        ...
        dist = (
            self.DIS_SURF2SURF
            - (self.DIS_ADS2SURF if a1 in adsorbate_atoms else 0)
            - (self.DIS_ADS2SURF if a2 in adsorbate_atoms else 0)
        )
    ...
    def check_system(self, atoms: Atoms, min_vacuum_ratio: float = 0.3) -> str:
    ...
    def generate_graph(
        self, atoms: Atoms, ads_indices: Optional[list[int]] = None, clean_substrate: Optional[Atoms] = None
    ) -> nx.Graph:
        """Generate a graph for the input atoms."""
        if ads_indices is None:
            raise Exception("The adsorbate indices must be provided.")

        # Create a graph with adsorbates.
        full = self._generate_structure_graph(atoms, ads_indices)
        input_ads_indices = copy.deepcopy(ads_indices)

        # Create a graph of clean substrate plus adsorbate graph to avoid the influence of surface relaxation.
        if clean_substrate is not None:
            # FIXME: need an alignment operation.
            self._print("use clean substrate plus adsorbates...")
            clean_graph = self._generate_structure_graph(clean_substrate, ads_indices)
    ...
    def _generate_structure_graph(self, atoms_: Atoms, ads_indices: list[int]):
        """generate molecular graph for reaction detection
        this is part of process_atoms
        """
        # Create a new graph when this method is called
        graph = nx.Graph()
        ...
        # Get the chemical symbols of all atoms used in node and edge notations
        chemical_symbols = atoms.get_chemical_symbols()

        # Add all atoms to graph as nodes
        natoms = len(atoms)
        for i in range(natoms):
            for x, y, z in grid_iterator(grid):
                self.add_atoms_node(graph, chemical_symbols[i], i, (x, y, z))

        # Create a neighbour list
        nl = self.neigh_creator.build_neighlist(atoms, bothways=False)
        nl.update(atoms)

        # Add all bonds to graph as edges
        for centre_idx in range(natoms):
            for x, y, z in grid_iterator(grid):
                ...
                for nei_idx, offset in zip(nei_indices, offsets):
                    ...
                    # This line ensures that only surface-adsorbate bonds are accounted for that are less than 2.5 Ang
                    dis = atoms.get_distances(centre_idx, nei_idx, mic=True)
                    if dis > self.substrate_adsorbate_distance and (
                        bool(centre_idx in ads_indices) ^ bool(nei_idx in ads_indices)
                    ):
                        continue
                    centre_sym, nei_sym = chemical_symbols[centre_idx], chemical_symbols[nei_idx]
                    self.add_atoms_edge(
                        graph,
                        centre_sym,
                        nei_sym,
...
def extract_chem_envs(
    ...
    ads_nodes = None
    ads_nodes = [node_symbol(chemical_symbols[i], i, (0, 0, 0)) for i in ads_indices]
    ...
    chem_envs = []
    for _, ads in enumerate(ads_graphs):
        ...
        full_ads = nx.ego_graph(
            full, initial, radius=0, distance="ads_only"
        )  # all nodes in this adsorbate, equal ads?
        # print("full ads: ", full_ads.nodes())

        new_ads = nx.ego_graph(
            full, initial, radius=(graph_radius * DIS_SURF2SURF) + DIS_ADS2SURF, distance="dist"
