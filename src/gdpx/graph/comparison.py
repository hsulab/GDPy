...
def get_unique_environments_based_on_nodes_and_edges(chem_envs):
    ...
    for env in chem_envs:
        for unique_env in unique:
            if nx.algorithms.isomorphism.is_isomorphic(env, unique_env, edge_match=bond_match, node_match=ads_match):
                break
        else:  # Was unique
            unique.append(env)
...
def unique_chem_envs(chem_envs_groups, metadata=None, verbose=False):
    ...
    if metadata is not None:
        if len(chem_envs_groups) != len(metadata):
            raise ValueError(
                "Metadata must be the same length as\
    ...
    for index, env in enumerate(chem_envs_groups):
        ...
        for index2, (unique_indices, unique_env) in enumerate(unique):
            if verbose:
                print(
                    "Checking for uniqueness {:05d}/{:05d} {:05d}/{:05d}".format(
                        index + 1, len(chem_envs_groups), index2, len(unique)
                    ),
                    end="\r",
                )
...
def paragroup_unique_chem_envs(chem_envs_groups, metadata=None, directory=pathlib.Path.cwd(), n_jobs=1):
    """"""
    ...
    if saved_info.exists():
        ...
        extra_info = [{"similar": [], "others": []} for i in range(nframes)]
    ...
    with Parallel(
        ...
        for icount in range(500):
            ...
            # run para cmp
            ret = para(
                delayed(new_unique_chem_envs)(extra_info, chem_envs_groups, env_indices)
                for env_indices in env_indices_splits
            )
