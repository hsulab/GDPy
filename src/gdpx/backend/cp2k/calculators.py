...
from .parser import read_cp2k_energy_force, read_cp2k_outputs, read_cp2k_scf_convergence
...
class Cp2kFileIO(FileIOCalculator):
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        if isinstance(prev_basis_set, str):
            curr_basis_set = {k: prev_basis_set for k in elements}
        ...
        # Support mixed pseudo_potential
        prev_pseudo_potential = self.parameters.pseudo_potential
        if isinstance(prev_pseudo_potential, str):
            curr_pseudo_potential = {k: prev_pseudo_potential for k in elements}
        elif isinstance(prev_pseudo_potential, dict):
            for k in elements:
                if k not in prev_pseudo_potential:
                    raise RuntimeError(f"No pseudo_potential for {k}.")
            curr_pseudo_potential = prev_pseudo_potential
        else:
            raise RuntimeError(f"Unknown pseudo_potential {prev_pseudo_potential}.")
        self.parameters.pseudo_potential = curr_pseudo_potential

        # Support DFT+U inputs
        prev_dft_plus_u = self.parameters.dft_plus_u
        if prev_dft_plus_u is not None:
            if isinstance(prev_dft_plus_u, dict):
                curr_dft_plus_u = {}
                for elem in elements:
                    if elem not in prev_dft_plus_u:
                        curr_dft_plus_u[elem] = None
                    else:
                        curr_dft_plus_u[elem] = prev_dft_plus_u[elem]
            else:
                raise Exception(f"Unknown dft_plus_u {prev_dft_plus_u}.")
            self.parameters.dft_plus_u = curr_dft_plus_u
    ...
    def _generate_input(self):
        ...
        # Write keywords in FORCE_EVAL/SUBSYS after checking existing sections,
        # which is necessary for the restart.
        subsys = root.get_subsection("FORCE_EVAL/SUBSYS").subsections
        subsys_names = [s.name for s in subsys]

        # write coords
        syms = self.atoms.get_chemical_symbols()
        atoms = self.atoms.get_positions()
        if "COORD" not in subsys_names:
        ...
        for elem in set(self.atoms.get_chemical_symbols()):
            if elem not in kinds.keys():
                ...
                if p.pseudo_potential:
                    kinds[elem].keywords.append("POTENTIAL " + p.pseudo_potential[elem])
                if p.dft_plus_u and p.dft_plus_u[elem] is not None:
                    kinds[elem].add_keyword(
                        "DFT_PLUS_U", "U_MINUS_J [eV]" + " " + str(p.dft_plus_u[elem]["u"] - p.dft_plus_u[elem]["j"])
                    )
                    kinds[elem].add_keyword("DFT_PLUS_U", "L" + " " + str(p.dft_plus_u[elem]["l"]))

        output_lines = ["!!! Generated by GDPX !!!"] + root.write()
        return "\n".join(output_lines)
