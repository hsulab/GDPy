...
class GridCalculator:
    ...
    def __init__(self, dpath, unit="eV", *args, **kwargs) -> None:
        ...
        self._gradients = np.gradient(
            z, np.unique(data[:, 0]), np.unique(data[:, 1])
        )
    ...
    def _preprocess_data(self, data):
        ...
        if (not np.all(np.abs(xdiff - xdiff[0]) < 1e-8)) or (
            not np.all(np.abs(ydiff - ydiff[0]) < 1e-8)
        ):
            warnings.warn(
...
class GridManager(BasePotentialManager):

    name = "grid"

    implemented_backends = ["grid"]
    valid_combinations = (("grid", "grid"),)
