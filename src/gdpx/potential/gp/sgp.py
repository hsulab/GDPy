...
def gaussian_kernel(x1, x2, delta=0.2, theta=0.5):
    ...
    x_diff = (
        x1[:, np.newaxis, :].repeat(x2.shape[0], axis=1) - x2[np.newaxis, :, :]
    )

    return delta**2 * np.exp(-np.sum(x_diff**2, axis=-1) / 2.0 / theta**2)
...
def gaussian_kernel_value_and_grad(x1, x2, delta=0.2, theta=0.5):
    ...
    v = delta**2 * np.exp(-(x_diff**2) / 2.0 / theta**2)
...
def xxx_gaussian_kernel_value_and_grad(x1, x2, delta=0.2, theta=0.5):
    ...
    v = delta**2 * np.exp(-np.sum(x_diff**2, axis=-1) / 2.0 / theta**2)
    ...
    for i_frame, atoms in enumerate(frames):
        ...
        for i in range(natoms):
            ...
            for j, offset in zip(nei_indices, nei_offsets):
                ...
                dis_deriv = np.hstack(
                    [pos_vec / distance, -pos_vec / distance]
                )
...
def compute_distance_kernal_matrices(
    ...
    kernels, kernel_derivs, kernel_grads_wrt_delta, _ = (
        gaussian_kernel_value_and_grad(distances, sparse_points, delta=delta)
    )
    # print("b2_kernels: ")
    # print(kernels)
    ...
    kernel_derivs = (
        dis_derivs[:, :, np.newaxis].repeat(num_sparse, axis=2)
        * kernel_derivs[:, np.newaxis, :]
    )
    ...
    for i in range(nframes):
        for dis_loc, kernel, k_grad in zip(
            distance_mapping_list, kernels, kernel_grads_wrt_delta
        ):
            if dis_loc[0] == i:
                Knm_ene[i, :] += kernel
    ...
    for dis_loc, kernel_grad in zip(distance_mapping_list, kernel_derivs):
        ...
        Knm_grad_grad_wrt_delta[i : i + 3, :] += (
            kernel_grad[0:3, :] * 2 / delta
        )
        Knm_grad_grad_wrt_delta[j : j + 3, :] += (
            kernel_grad[3:6, :] * 2 / delta
        )
    Knm_frc = -Knm_grad
    ...
    return (
        ...
        Kmm_grad_wrt_delta,
        Knm,
        Knm_ene_grad_wrt_delta,
        Knm_grad_grad_wrt_delta,
    )
...
def compute_distance_marginal_likelihood(
    ...
    (
        ...
        Knm_ene_grad_wrt_delta,
        Knm_grad_grad_wrt_delta,
    ) = compute_distance_kernal_matrices(
        delta,
        sigma2,
        nframes,
        natoms_list,
    ...
    loss = (
        -0.5 * np.log(np.linalg.det(Knn))
        - 0.5 * y_data.T @ Knn_inv @ y_data
        - num_points / 2.0 * np.log(2 * np.pi)
    )

    # -- get gradients
    Knm_grad_wrt_delta = np.vstack(
        [Knm_ene_grad_wrt_delta, -Knm_grad_grad_wrt_delta]
    )
    Kmm_inv_grad_wrt_delta = -Kmm_inv @ Kmm_grad_wrt_delta @ Kmm_inv
    Ky = Knn_inv @ y_data

    Knn_grad_wrt_delta = (
        Knm_grad_wrt_delta @ Kmm_inv + Knm @ Kmm_inv_grad_wrt_delta
    ) @ Knm.T + Knm @ Kmm_inv @ Knm_grad_wrt_delta.T
...
def gaussian_kernel_value_and_grad_body3(x1, x2, delta=0.2, theta=0.5):
    ...
    v = delta**2 * np.exp(-(x_norm**2) / 2.0 / theta**2)
...
def compute_body3_kernel_matrices(
    ...
    for p in itertools.permutations(range(3), 3):
        ...
        b3_kernels += _k * b3s
        b3_kernel_grad_x1 += (
            _k_g_x1 * b3s[:, :, np.newaxis]
            + np.repeat(_k[:, :, np.newaxis], 3, axis=2) * b3sg
        )
        b3ks_gdelta += k_gdelta * b3s
        b3ks_gtheta += k_gtheta * b3s
        # --
        x_norm = np.linalg.norm(
            b3_features[:, np.newaxis, :].repeat(num_sparse, axis=1)
            - sparse_body3_features[:, p],
            axis=-1,
        )[:, :, np.newaxis]
        # b3ks_gx1_gtheta_coef += (-2./theta + x_norm**2/theta**3)
        b3ks_gx1_gtheta += (
            -2.0 / theta * _k_g_x1 * b3s[:, :, np.newaxis]
            + x_norm**2 / theta**3 * b3_kernel_grad_x1
        )
    ...
    for loc, kernel, k_gdelta, k_gtheta in zip(
        body3_mapping, b3_kernels, b3ks_gdelta, b3ks_gtheta
    ):
        fi, i, j, k = loc
    ...
    for loc, b3_grad, k_grad, k_gx1gtheta in zip(
        ...
        # -- PASS
        Knm_gcart_gdelta[i * 3 : i * 3 + 3, :] += (
            curr_grad[:, 0, 0:3].T * 2.0 / delta
        )
        Knm_gcart_gdelta[j * 3 : j * 3 + 3, :] += (
            curr_grad[:, 0, 3:6].T * 2.0 / delta
        )
        Knm_gcart_gdelta[i * 3 : i * 3 + 3, :] += (
            curr_grad[:, 1, 0:3].T * 2.0 / delta
        )
        Knm_gcart_gdelta[k * 3 : k * 3 + 3, :] += (
            curr_grad[:, 1, 3:6].T * 2.0 / delta
        )
        Knm_gcart_gdelta[j * 3 : j * 3 + 3, :] += (
            curr_grad[:, 2, 0:3].T * 2.0 / delta
        )
        Knm_gcart_gdelta[k * 3 : k * 3 + 3, :] += (
            curr_grad[:, 2, 3:6].T * 2.0 / delta
        )
        # --
        ...
        curr_grad_theta = (
            b3_grad[:, np.newaxis, :] * k_gx1gtheta.T[:, :, np.newaxis]
        )
    ...
    for p in itertools.permutations(range(3), 3):
        # for p in [(0, 1, 2)]:
        curr_Kmm, _, curr_Kmm_gdelta, curr_Kmm_gtheta = (
            gaussian_kernel_value_and_grad_body3(
                sparse_body3_features,
                sparse_body3_features[:, p],
                delta=delta,
                theta=theta,
...
def compute_body2_kernel_matrices(
    ...
    # -
    _b2_kernels, _b2_kernel_grad_x1, b2_kg_wrt_delta, b2_kg_wrt_theta = (
        gaussian_kernel_value_and_grad(
    ...
    b2_kernel_grad_x1 = (_b2_kernel_grad_x1 * b2s + _b2_kernels * b2sg)[
        :, :, np.newaxis
    ]  # (num_b2, num_sparse, 1)
    # print(b2_kernel_grad_x1)
    # print(b2_kernel_grad_x1.shape)

    b2_kg_wrt_delta = (b2_kg_wrt_delta * b2s)[
        :, :, np.newaxis
    ]  # (num_b2, num_sparse, 1)
    b2_kg_wrt_theta = (b2_kg_wrt_theta * b2s)[
        :, :, np.newaxis
    ]  # (num_b2, num_sparse, 1)
    print(_b2_kernels)
    ...
    Knm_ene_gtheta = np.zeros((nframes, num_sparse))
    for loc, kernel, k_gdelta, k_gtheta in zip(
        body2_mapping, b2_kernels, b2_kg_wrt_delta, b2_kg_wrt_theta
    ):
        fi, i, j = loc
        Knm_ene[fi, :] += kernel
    ...
    b2ks_gx1_gtheta = (
        -2.0 / theta * _b2_kernel_grad_x1 * b2s
        + x_norm**2 / theta**3 * b2_kernel_grad_x1.squeeze()
    )[:, :, np.newaxis]
    ...
    for loc, b2_grad, k_grad, b2k_gx1gtheta in zip(
        ...
        # print(xxx.shape)
        # exit()
        Knm_gcart_gtheta[i * 3 : i * 3 + 3, :] += (
            b2_grad[:, 0:3] * b2k_gx1gtheta
        ).T
        Knm_gcart_gtheta[j * 3 : j * 3 + 3, :] += (
            b2_grad[:, 3:6] * b2k_gx1gtheta
        ).T
    Knm_frc = -Knm_grad
...
def compute_b2_marginal_likelihood(
    ...
    Knn_grad_wrt_theta = (
        Knm_grad_wrt_theta @ Kmm_inv + Knm @ Kmm_inv_grad_wrt_theta
    ) @ Knm.T + Knm @ Kmm_inv @ Knm_grad_wrt_theta.T
...
def train_ene(
    ...
    # - train ene
    Kmm = np.zeros(
        (num_b2_sparse + num_b3_sparse, num_b2_sparse + num_b3_sparse)
    )
    ...
    loss = (
        -0.5 * (np.log(sign) + logabsdet)
        - 0.5 * y_data_ene.T @ Knn_inv @ y_data_ene
        - nframes / 2.0 * np.log(2 * np.pi)
    )
    ...
    lg_b2delta = compute_loss_gradient(
        Ky, Knn_inv, Kmm_inv, Kmm_gdelta, Knm, Knm_gdelta
    )
    ...
    lg_b2theta = compute_loss_gradient(
        Ky, Knn_inv, Kmm_inv, Kmm_gtheta, Knm, Knm_gtheta
    )
    ...
    lg_b3delta = compute_loss_gradient(
        Ky, Knn_inv, Kmm_inv, Kmm_gdelta, Knm, Knm_gdelta
    )
    ...
    lg_b3theta = compute_loss_gradient(
        Ky, Knn_inv, Kmm_inv, Kmm_gtheta, Knm, Knm_gtheta
    )
...
def compute_loss_gradient(Ky, Knn_inv, Kmm_inv, Kmm_gdelta, Knm, Knm_gdelta):
    ...
    Kmm_inv_grad_wrt_delta = -Kmm_inv @ Kmm_gdelta @ Kmm_inv
    Knn_grad_wrt_delta = (
        Knm_gdelta @ Kmm_inv + Knm @ Kmm_inv_grad_wrt_delta
    ) @ Knm.T + Knm @ Kmm_inv @ Knm_gdelta.T
...
class SparseGaussianProcessTrainer:
    ...
    def run(self, *args, **kwargs):
        ...
        res = optimize.minimize(
            compute_b2b3_marginal_likelihood,
            [delta, theta, delta, theta],
            args=(
                sigma2,
                self.r_cut,
    ...
    def run_b2(self, *args, **kwargs):
        ...
        # --- body2
        print("~~~~~ USE BODY-2 ~~~~~")
        sparse_body2_features = np.array([0.5, 1.0, 1.5, 2.0, 2.5])[
            :, np.newaxis
        ]
        Kmm_b2, Knm_b2, _, _, _, _, _, _ = compute_body2_kernel_matrices(
            delta,
        ...
        res = optimize.minimize(
            compute_b2_marginal_likelihood,
            [delta, theta],
            args=(
                sigma2,
                self.r_cut,
    ...
    def run_b3(self, *args, **kwargs):
        ...
        res = optimize.minimize(
            compute_b3_marginal_likelihood,
            [delta, theta],
            args=(
                sigma2,
                self.r_cut,
    ...
    def _prev_run2(self, *args, **kwargs):
        ...
        # - get atomic environments
        distances, distance_mapping_list, dis_derivs = (
            compute_body2_descriptor(frames, self.r_cut)
        )
        ...
        # - train
        res = optimize.minimize(
            compute_distance_marginal_likelihood,
            delta,
            args=(
                sigma2,
                nframes,
    ...
    def _train_and_predict(self, nframes, sigma2_matrix, y_data, Kmm, Knm):
        ...
        print("ERR: ")
        ene_rmse = np.sqrt(
            np.sum((dft_data[:nframes] - sgp_data[:nframes]) ** 2)
        )
        ene_svar = np.sqrt(
            np.var((dft_data[:nframes] - sgp_data[:nframes]) ** 2)
        )
...
class SGPTrainer(BasePotentialTrainer):
