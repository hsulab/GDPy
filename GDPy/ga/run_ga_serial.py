...
STEPS, FMAX, ZMIN = 500, 0.05, 2.3

def perform_optimisation(atoms):
    calc.reset()
    saved_info = atoms.info
    atoms.calc = calc
    atoms, min_stat = atoms.calc.minimise(
        atoms, steps=STEPS, fmax=FMAX, zmin=ZMIN
    )
    atoms.info = saved_info
    print(min_stat)
