...
def molecule():
    ...
    atoms = Atoms(
        symbols="CO2",
        positions=[[0.0, 0.0, 0.0], [1.2, 0.01, 0.0], [-1.2, 0.0, 0.0]],
        cell=np.eye(3) * 20.0,
        pbc=True,
    )
...
class TestInternalCoordinates:
    ...
    def test_iterative_position_update(self, molecule: Atoms):
        ...
        pairs = np.array([[0, 1], [0, 2]])
        trimers = np.array([[0, 1, 2]])
        targets = np.array([1.5, 1.3, 3.14 / 3.0])
        ...
        results = np.array(
            [
                [-0.09825885, -0.7998146, 0.0],
                [0.7534894, 0.43493205, 0.0],
                [-0.65523046, 0.37488258, 0.0],
            ]
        )
...
class TestHyperCubeBuilder:
    ...
    def test_build_molecules(self, molecule):
        ...
        builder = HypercubeBuilder(
            ...
            disrange=[0.8, 1.6],
            angles=[[0, 1, 2]],
            angrange=[75.0 / 180.0 * np.pi, 165.0 / 180.0 * np.pi],
            random_seed=1112,
        )
        ...
        positions = np.array(
            [
                [0.00358386, -0.20563598, 0.0],
                [1.14606953, 0.11257479, 0.0],
                [-1.14965343, 0.10306119, 0.0],
            ]
        )
