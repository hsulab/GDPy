...
from gdpx.core.session.interface import create_variable
...
def xxxx():
    OmegaConf.register_new_resolver("structure", lambda x: read(x, ":"))
...
def sdasdsa_main():
    ...
    OmegaConf.register_new_resolver("read_json", read_json)  # lambda x: read_json(x)
...
def main():
    ...
    OmegaConf.register_new_resolver(
        "gdp_v", lambda x: create_node("xxx", OmegaConf.to_object(x)), use_cache=False
    )
    OmegaConf.register_new_resolver(
    ...
    OmegaConf.register_new_resolver(
        ...
        lambda x: build(
            *[
                create_node(
                    "xxx", {"type": "builder", "method": "molecule", "filename": x}
                )
            ]
        ),
    )
